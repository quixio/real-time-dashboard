...
echo 'Checking if python 3.6.8 is installed'
installPython=true
which -s python3 > /dev/null 2>&1
if [ $? = 0 ]
then
    pythonVerison="$(python3 -V)"
    if [ "$pythonVerison" != "Python 3.6.8" ]
    then
      # TODO maybe look up what version it is linked to and try to update to 3.6 and hope for the best?
      echo "Found python, but not 3.6.8, it is $pythonVerison"
    else
        installPython=false
    fi
fi
...
then
    echo 'Python 3.6.8 is not installed, downloading and installing it. This may take a while'
    mkdir -p downloads
    curl https://www.python.org/ftp/python/3.6.8/python-3.6.8-macosx10.9.pkg --output ./downloads/python3.6.8.pkg
    echo 'Python 3.6.8 downloaded. Installing now.'
    sudo installer -verbose -pkg downloads/python3.6.8.pkg -target /
    if [ $? != 0 ] ; then printf "\e[91m╳ Python failed to install.\n\e[0m" ; exit 1 ; fi ;
    printf "\e[32m✓ Python 3.6.8 is now installed\n\e[0m"
else
    printf "\e[32m✓ Python 3.6.8 is already installed\n\e[0m"
fi
